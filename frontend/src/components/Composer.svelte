...
<script lang="ts">
  ...
  let modoCodex = $state<'default' | 'plan' | null>(null);
  ...
  $effect(() => {
    ...
    getCodexModels(sn)
      .then((res) => {
        ...
        modoCodex = res.current.mode ?? null;
      })
      .catch(() => {
...
<footer class="composer" class:compacto>
  ...
  <input
    ...
    tabindex="-1"
  />
  <div class="composer-dock" use:posicionarModo>
  ...
  {#if temAba || ((isClaude || isCodex) && modoNaPrimeiraLinha)}
