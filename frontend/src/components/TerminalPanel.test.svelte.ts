...
// invisível.
//
// B1 — o Shell relia `servidorAtivo()` DEPOIS do await do POST: trocando o ativo no meio, o POST
//       criava term-<nome> em A e o socket abria em B.
...
// grupo — nenhum teste toca rede; a prova é a URL que o socket monta).
...
beforeEach(() => {
  ...
  // fetchSessionsForServer (usado pelo probe) fake; openShell (POST) fake e adiado.
...
describe('TerminalPanel — bloqueadores da Task 2', () => {
  it('B1: o socket do Shell usa o servidor capturado ANTES do await, mesmo trocando o ativo no meio', async () => {
    ...
    await frames(2);
    // enquanto o POST espera, o usuário troca o ativo para B
    ...
    // O socket deve usar o servidor capturado ANTES do await (A), não o novo ativo (B)
    expect(url).toContain(`ws://${srvA.baseUrl.replace('http://', '')}/api/sessions/term-s/term?`);
    expect(url).toContain('token=ta');
    expect(url).not.toContain('ws://b');
    unmount(comp);
  });
