<script lang="ts">
  ...
  async function escolher(v: EstiloDitado) {
    ...
    try {
      ...
      // achar que trocou.
      err = e instanceof Error ? e.message : m.config_motores_erro_salvar();
      aplicando = null;
      return;
    }
    aplicando = null;
...
<Popover {open} {anchor} {onClose} width={230} ariaLabel={m.ditado_estilo_titulo()}>
  {#if err}
    <p class="err" role="alert">{err}</p>
  {/if}

  <ul class="lista">
    {#each estilosDitado() as e (e.valor)}
      <li>
        <button
          class="linha"
  ...
  <p class="dica">{m.ditado_estilo_dica()}</p>
</Popover>
