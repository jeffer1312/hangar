// Formatação num lugar só: vírgula decimal, ponto de milhar. Espalhar `toFixed()` pela
// tela foi o que produziu "19.83 Bi" ao lado de "US$ 17.730,43" — dois significados pro mesmo
// ponto na mesma página. O locale vem do idioma ESCOLHIDO no app (intlLocale), nao do navegador.
export type Cur = 'USD' | 'BRL';

import { intlLocale } from './locale';

export const dec = (n: number, casas: number) =>
  n.toLocaleString(intlLocale(), { minimumFractionDigits: casas, maximumFractionDigits: casas });
...
const nf = (cur: Cur, extra: Intl.NumberFormatOptions = {}) =>
  new Intl.NumberFormat(intlLocale(), { style: 'currency', currency: cur, ...extra });

// `style: 'currency'` do ICU separa símbolo e número com NBSP (U+00A0), nunca com espaço
// normal. Medido no locale pt-BR: Intl.NumberFormat('pt-BR',{style:'currency',currency:'USD'}).format(105336.79)
