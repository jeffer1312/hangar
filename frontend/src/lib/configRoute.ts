...
export type TelaConfig = 'root' | 'geral' | 'aparencia' | 'ditado' | 'notificacoes' | 'anexos' | 'avancado' | 'motores' | 'sobre' | 'diario' | 'servidores' | 'acesso' | 'contas';

const TELAS: readonly TelaConfig[] = ['root', 'geral', 'aparencia', 'ditado', 'notificacoes', 'anexos', 'avancado', 'motores', 'sobre', 'diario', 'servidores', 'acesso', 'contas'];

/** Telas que mexem NUM servidor — sem alvo resolvido elas nao podem abrir (ver App.svelte). */
export const TELAS_DE_SERVIDOR: readonly TelaConfig[] = ['acesso', 'contas', 'notificacoes', 'anexos', 'avancado', 'motores'];
