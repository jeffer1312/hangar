...
// `tipo` vem da união fechada de OrqEvento, não `string`: tipo novo no backend, ou um literal
// digitado errado num teste, tem que virar erro de compilação aqui.
export function etapaAtual(eventos: { tipo: OrqEvento['tipo']; resultado?: string }[]): Etapa {
