// Gera packages/core/src/fileIcons.generated.ts a partir do material-icon-theme (node_modules),
// só os nomes de packages/core/src/fileIcons.lista.json. O script fica aqui porque o
// material-icon-theme é dependência de desenvolvimento do frontend; o ícone em si é do core, que
// a PWA e o app leem igual. O gerado é commitado: o build da VPS não instala o pacote.
// Rodar: `npm run icons`.
...
const destino = join(dirname(raiz), 'packages', 'core', 'src');
const lista = JSON.parse(readFileSync(join(destino, 'fileIcons.lista.json'), 'utf8'));
...
writeFileSync(join(destino, 'fileIcons.generated.ts'),
