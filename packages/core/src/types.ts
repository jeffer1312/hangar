...
export interface ChatEvent {
  ...
  /** Só em bolha "queued-": relógio da entrada (epoch s), pra ordenar quando chega ao vivo. */
...
export interface StateEvent {
  ...
  /** Claude sem terminal com `ExitPlanMode` aguardando aprovação: o plano que será aprovado. */
  claude_plan_pending?: { plan: string; path: string | null; tool_use_id: string | null } | null;
...
export interface PreviewEvent { text?: string; md?: boolean; full?: boolean; vivo?: boolean }
...
export interface Atualizacao {
  ...
  versao_legivel?: { repo: string | null; backend: string | null; remoto: string | null };
