...
- O fallback de `window` usa `stopImmediatePropagation()` (além de `preventDefault`) para impedir listeners irmãos no próprio `window`, e continua fechando a sheet quando o foco está fora do conteúdo.
