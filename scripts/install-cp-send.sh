...
- Criar sessão nova (usuário pediu, ou a tarefa precisa de par em outro repo): `cp-send --new <nome> <cwd>` — NUNCA `tmux new-session` cru (fica sem --session-id, invisível no app). Criar por iniciativa própria → avisar o usuário no terminal o porquê. As escolhas de abertura (combináveis) são flags do `--new`, não instalações à parte:
  ...
  - **MODELO, ESFORÇO e PERMISSÃO de nascença** — `--model <id>`, `--effort <nível>`, `--permissao <modo>`: a sessão já ABRE assim, sem gastar um turno no default errado. Tarefa com modelo definido pelo usuário ("abre no Opus em high") → `cp-send --new <nome> <cwd> --conta X --model opus --effort high`. SEMPRE preferir isso a pedir `/model` no kick-off: a sessão trabalha ANTES de ler o recado. Modelo aceita alias ou id (`opus`, `sonnet`, `haiku`, `claude-opus-5`…); esforço: `low|medium|high|xhigh|max` (claude; no pi vira `--thinking`, que aceita também `off|minimal`; kimi só `--model`); permissão (só claude): `acceptEdits|auto|bypassPermissions|manual|dontAsk|plan`. Valor inválido → 400 do backend e a sessão NÃO nasce.
