...
let kimiUsageCache: { text: string; fetchedAt: number } | null = null;

function kimiUsageSummary(): string {
	const now = Date.now();
	if (kimiUsageCache && now - kimiUsageCache.fetchedAt < 300_000) {
		return kimiUsageCache.text;
	}
	try {
		const auth = JSON.parse(readFileSync(PI_AUTH_PATH, "utf8"));
		const token = auth?.["kimi-coding"]?.access ?? auth?.["kimi-coding"]?.key;
		if (!token) return "";
		const body = execFileSync(
			"curl",
			[
		...
		const data = JSON.parse(body);
		const pct = (d: any) =>
			d?.limit ? Math.round((Number(d?.used ?? 0) / Number(d.limit)) * 100) : "?";
		const resetIn = (d: any) =>
			d?.resetTime
				? formatDurationSeconds(Math.max(0, Math.floor((new Date(d.resetTime).getTime() - now) / 1000)))
				: "?";
		const fiveHour = data?.limits?.find(
			(l: any) => l?.window?.timeUnit === "TIME_UNIT_MINUTE" && l?.window?.duration === 300,
		)?.detail;
		const weekly = data?.usage;
		const fiveHourText = fiveHour ? `⚡5h:${pct(fiveHour)}% ↻${resetIn(fiveHour)}` : "⚡5h:ok";
		const weeklyText = weekly ? `📅7d:${pct(weekly)}% ↻${resetIn(weekly)}` : "📅7d:?";
		const text = `${fiveHourText} ${weeklyText}`;
		kimiUsageCache = { text, fetchedAt: now };
		return text;
	} catch {
		return kimiUsageCache?.text ?? "";
	}
...
export default function (pi: ExtensionAPI) {
	...
	function renderFooter(
		...
		const kimiLimitText = ctx.model?.provider === "kimi-coding" ? kimiUsageSummary() : "";
