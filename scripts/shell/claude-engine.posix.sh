...
claude-engine() {
    if [ "$#" -eq 0 ]; then
        ...
        # hangar-engine — distingue "não instalado" (comando não encontrado) de "zero motor configurado"
        ...
        if ! lista=$(hangar-engine --list 2>/dev/null); then
