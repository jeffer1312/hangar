...
- Criar sessão nova (usuário pediu, ou a tarefa precisa de par em outro repo): `hangar-send --new <nome> <cwd>` — NUNCA `tmux new-session` cru (fica sem --session-id, invisível no app). Criar por iniciativa própria → avisar o usuário no terminal o porquê. As escolhas de abertura (combináveis) são flags do `--new`, não instalações à parte:
  - **Outro AGENTE** — `--provider <claude|codex|pi|omp|kimi>`: qual CLI sobe no pane. Pedido "abre uma sessão no Kimi/Codex/Pi" → `hangar-send --new <nome> <cwd> --provider kimi` (ou codex/pi). `--provider omp` abre o oh-my-pi (fork do Pi; mesmas extensões). NÃO precisa de motor nem config extra — só do binário instalado. Recado, fila, pareamento e grupo funcionam igual entre providers.
