...
- Outras sessões Claude vivas nesta máquina: `hangar-send --list` (nome, estado, harness, cwd). Mandar recado: `hangar-send <sessao> "msg"` — chega como prompt lá (fila durável se ocupada). Referência completa e sempre atual dos comandos: `hangar-send --help`.
- **MCP `hangar`** (quando as tools `who_am_i`/`sessions`/`send`/`group`/`pair`/`unpair`/`new_session`/`browser_open`/`browser`/`browser_batch` existirem na sessão): são o `hangar-send` e o `hangar-preview` como tool tipada, sem Bash — prefira-as; as regras abaixo valem igual. O que não tem tool (`folha`, `--sessao` de outra) continua no CLI. Sem as tools (sessão aberta antes do registro, Pi/omp/Kimi), tudo é CLI. Os nomes antigos em português (`quem_sou`, `sessoes`, `enviar`, `grupo`, `parear`, `desparear`, `nova_sessao`, `nav_abrir`, `nav`, `nav_lote`) ainda respondem, pra sessão aberta antes do rename não quebrar; em conversa nova, use os de cima.
- **`new_session` nasce na conta de quem chama e devolve o `config_dir` que usou — CONFIRA esse campo.** Outra conta: `conta` com o caminho do config dir, ou `hangar-send --new --conta <nome>` quando ela ainda precisar ser preparada.
