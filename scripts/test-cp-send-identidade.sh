#!/usr/bin/env bash
# Trava a resolução de identidade do cp-send (a função `me`), com um `tmux` FALSO no PATH — mesmo
...
#      renomear pelo app (que é FEATURE) deixava o carimbo obsoleto na hora. O cp-send caía no
...
#      há exatamente uma.
#
# Uso: ./scripts/test-cp-send-identidade.sh
set -uo pipefail
...
# `me` sai do cp-send de verdade: testar uma cópia colada aqui passaria com o arquivo real quebrado.
sed -n '/^me() {/,/^}/p' "$REPO/scripts/cp-send" > "$TMP/me.sh"
[[ -s "$TMP/me.sh" ]] || { echo "FALHA: não achei a função me() em scripts/cp-send"; exit 1; }
