...
## Proof of seeing — without it, a BLOCKER
...
The executor's protocol — every executor, sighted or not — is in `executor-visual.md`. A
screenshot from before the fix doesn't count: if they fixed it, they must have recaptured.
...
the same state and width. The round message's `Visual:` line points at the executor's visual
report, which must carry, per round: who won, **which letter was their work**, the biggest hole
named and what they fixed. A report saying only "I compared and it looks good" is the same "does
it look good?" in other clothes — it blocks.
...
- **The bar swapped midway** — your checklist is the four invalidators of `executor-visual.md`,
  step 3: viewport, language, edge-cut element, effect framed with the state. A comparison against
  the wrong bar is false evidence, not weak evidence.
...
Redoing the comparison round after round yields divergences, not blockers — the blockers come from
the code.
...
- **With the round cap met, the bar CLOSES** — and closed means nobody redoes it, you included.
  Whatever aesthetic divergence remains becomes `NOTED`. They lost both rounds and sent the round
...
**A round that touches no pixel doesn't pay the bar again.** A correction round that only
touches store, tests or backend redoes no comparison — `git stash show --stat <object>` proves it,
and your window goes entirely to the code.
...
Don't follow screenshot by screenshot while the work moves. Whoever captures, describes — the
executor and your verification session can both see (a vision command or a vision subagent,
whatever the contract's tooling names). Let the two work and, **at the end, open ALL the
screenshots at once** and check that each shows what you needed. One pass of yours, at the end,
over the set — not a read of yours per image.

And **a symbol or color claim is checked zoomed**, against the crop, never by eye on the whole
image.
