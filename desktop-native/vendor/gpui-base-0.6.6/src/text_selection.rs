// Modified for Hangar (see ../MODIFICADO.md): participants of a cached view
// that was reused this frame survive the end-of-frame sweep.
use std::{
...
pub struct TextSelectionRegistration {
    /// The view that painted the participant, for [`TextSelection::retain_cached_view`].
...
impl TextSelectionHandle {
    ...
    pub fn register(
        ...
        registration.view = Some(window.current_view());
...
struct WindowSelectionState {
    ...
    /// Views that rendered again this frame; their unpainted participants are swept.
    rendered_views: HashSet<EntityId>,
...
impl WindowSelectionState {
    ...
    pub fn finish_frame(&mut self, cx: &mut App) -> Vec<ClearHandler> {
        ...
        self.rendered_views.clear();
        handlers
    }

    /// Restamps the participants of a cached view reused this frame: its
    /// elements did not paint, so they did not register, but they are on screen.
    /// They take the next document orders, keeping their order among themselves.
    fn retain_view(&mut self, view: EntityId, cx: &mut App) {
        if self.rendered_views.contains(&view) {
            return;
        }
        let mut stale = self
            .participants
            .iter()
            .filter(|(_, r)| {
                r.registration.view == Some(view) && r.generation != self.frame_generation
            })
            .map(|(id, r)| (r.registration.document_order, *id))
            .collect::<Vec<_>>();
        stale.sort();
        for (_, id) in stale {
            let order = GlobalState::global_mut(cx).next_selection_document_order();
            if let Some(participant) = self.participants.get_mut(&id) {
                let mut registration = (*participant.registration).clone();
                registration.document_order = order;
                participant.registration = Rc::new(registration);
                participant.generation = self.frame_generation;
            }
        }
    }
...
impl TextSelection {
    /// Marks a cached view as rendered again this frame. Call from its
    /// `render`; without it, a view that re-rendered with no selectable text
    /// would look reused and keep the participants it no longer shows.
    pub fn view_rendered(view: EntityId, window: &Window, cx: &mut App) {
        if let Some(state) = WindowSelectionState::existing(window, cx) {
            state.update(cx, |state, _| {
                state.rendered_views.insert(view);
            });
        }
    }

    /// Keeps the text of a cached view that was reused this frame selectable.
    /// Call during paint, right after the view paints: a view not in the frame
    /// never calls it, and its participants are swept as usual.
    pub fn retain_cached_view(view: EntityId, window: &mut Window, cx: &mut App) {
        if let Some(state) = WindowSelectionState::existing(window, cx) {
            state.update(cx, |state, cx| state.retain_view(view, cx));
        }
    }
...
mod tests {
    ...
    // Modified for Hangar: a cached view reused this frame keeps its participants; a view that
    // rendered again loses the ones it no longer paints.
    #[gpui::test]
    fn reused_cached_view_keeps_its_participants(cx: &mut TestAppContext) {
        let window = cx.add_window(|_, cx| WindowSelectionView {
            selection: TextSelectionHandle::new("unused", cx),
        });
        window
            .update(cx, |_, _, cx| {
                GlobalState::init(cx);
                let state = cx.new(|_| WindowSelectionState::default());
                let kept = FakeParticipant::new("kept", cx);
                let gone = FakeParticipant::new("gone", cx);
                let (cached, redrawn) = (cx.new(|_| ()).entity_id(), cx.new(|_| ()).entity_id());
                let registration = |view: EntityId, y: f32| {
                    let bounds = Bounds::new(point(px(0.), px(y)), size(px(100.), px(10.)));
                    let mut registration = TextSelectionRegistration::new(
                        Hitbox {
                            id: HitboxId::placeholder(),
                            bounds,
                            content_mask: ContentMask { bounds },
                    ...
                    .with_text_bounds(vec![bounds]);
                    registration.view = Some(view);
                    registration
                };
                state.update(cx, |state, cx| {
                    state.register_participant(kept.selection.clone(), registration(cached, 0.), cx);
                    state.register_participant(gone.selection.clone(), registration(redrawn, 20.), cx);
                    state.finish_frame(cx);
                    // Next frame: `cached` is reused and paints nothing; `redrawn` renders without its text.
                    state.rendered_views.insert(redrawn);
                    state.retain_view(cached, cx);
                    state.retain_view(redrawn, cx);
                    state.finish_frame(cx);
                    assert!(state.participants.contains_key(&kept.selection.entity_id()));
                    assert!(!state.participants.contains_key(&gone.selection.entity_id()));
                });
            })
            .unwrap();
    }
