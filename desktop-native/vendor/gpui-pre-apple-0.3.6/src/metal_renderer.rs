// Modificado pelo Hangar: desfoque portado de zeronsh/zui 18a89af.
...
impl MetalRenderer {
    ...
    fn draw_primitives_to_texture(
        ...
        for batch in scene.batches() {
            while pending_blurs
                .peek()
                .is_some_and(|blur| blur.order <= batch.first_order(scene))
            {
                command_encoder = self.draw_backdrop_blur(
                    pending_blurs.next().unwrap(),
                    writer,
                    texture,
                    viewport_size,
                    command_buffer,
    ...
    fn draw_backdrop_blur<'a>(
        ...
        let y1 = ((visible.origin.y.0 + visible.size.height.0 + padding).ceil() as i64)
            .min(drawable_height);
        if x1 <= x0 || y1 <= y0 {
            return Ok(command_encoder);
        }

        command_encoder.end_encoding();
        let binding = writer.write(std::slice::from_ref(blur))?;
        ...
        blit.copy_from_texture(
            ...
            0,
            0,
            metal::MTLOrigin { x: copy_x, y: copy_y, z: 0 },
            metal::MTLSize {
                width: scratch.width(),
    ...
    fn ensure_backdrop_scratch(
        ...
        if let Some(index) = self.backdrop_textures.iter().position(|pair| {
            pair.scratch.width() == width && pair.scratch.height() == height
        }) {
            let pair = self.backdrop_textures.remove(index);
            ...
            while !self.backdrop_textures.is_empty()
                && (self.backdrop_textures.len() >= 4
                    || self.backdrop_textures.iter().map(BackdropTextures::bytes).sum::<u64>()
                        + needed > budget)
            {
            ...
            self.backdrop_textures.push(BackdropTextures { scratch, blurred });
        }
        let budget = (drawable_width * drawable_height * 16)
            .min(32 * 1024 * 1024)
            .max(width * height * 8);
        while self.backdrop_textures.len() > 1
            && self.backdrop_textures.iter().map(BackdropTextures::bytes).sum::<u64>() > budget
        {
            self.backdrop_textures.remove(0);
        }
