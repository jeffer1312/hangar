...
use gpui::{
    ...
    StyleRefinement, Styled, WeakEntity, Window, anchored, deferred, div, prelude::FluentBuilder, px, rems,
};
...
// Modified for Hangar: presentation is opt-in; interaction remains owned by the menu.
/// Optional presentation for a popup menu's explicit parts.
#[derive(Default)]
pub struct PopupMenuAppearance {
    surface: StyleRefinement,
    list: StyleRefinement,
    item: StyleRefinement,
    separator: StyleRefinement,
    item_height: Option<Pixels>,
    label: Option<Box<dyn Fn(SharedString, &mut Window, &mut App) -> AnyElement>>,
    key: Option<Box<dyn Fn(Kbd, &mut Window, &mut App) -> AnyElement>>,
}

impl PopupMenuAppearance {
    /// Refine the outer surface, after the theme defaults.
    pub fn surface_style(mut self, style: StyleRefinement) -> Self { self.surface = style; self }
    /// Refine the items container, including its padding and gap.
    pub fn list_style(mut self, style: StyleRefinement) -> Self { self.list = style; self }
    /// Refine actionable rows, preserving their state styling.
    pub fn item_style(mut self, style: StyleRefinement) -> Self { self.item = style; self }
    /// Refine separators, after the theme defaults.
    pub fn separator_style(mut self, style: StyleRefinement) -> Self { self.separator = style; self }
    /// Set the standard row height and custom/submenu minimum height.
    pub fn item_height(mut self, height: Pixels) -> Self { self.item_height = Some(height); self }
    /// Render section labels with the renderer owning all internal padding.
    pub fn label_renderer<E: IntoElement>(mut self, render: impl Fn(SharedString, &mut Window, &mut App) -> E + 'static) -> Self {
        self.label = Some(Box::new(move |label, window, cx| render(label, window, cx).into_any_element()));
        self
    }
    /// Render the shortcut resolved through the menu's existing action context.
    pub fn key_renderer<E: IntoElement>(mut self, render: impl Fn(Kbd, &mut Window, &mut App) -> E + 'static) -> Self {
        self.key = Some(Box::new(move |key, window, cx| render(key, window, cx).into_any_element()));
        self
    }
}
...
impl PopupMenu {
    ...
    /// Opt into presentation overrides without replacing menu interaction.
    pub fn appearance(mut self, appearance: PopupMenuAppearance) -> Self {
        self.appearance = Some(appearance);
        self
    }

    /// Replace one item without resetting selection or rebuilding sibling submenus.
    /// Returns false when the index is out of bounds.
    pub fn replace_item(&mut self, index: usize, item: PopupMenuItem, cx: &mut Context<Self>) -> bool {
        let Some(current) = self.menu_items.get_mut(index) else { return false; };
        *current = item;
        cx.notify();
        true
    }
    ...
    // Modified for Hangar: títulos não consomem o primeiro movimento do teclado.
    fn first_selectable(&self) -> usize {
        if self.appearance.is_some() {
            self.clickable_menu_items().next().map(|(ix, _)| ix).unwrap_or(0)
        } else { 0 }
    }
    ...
    fn select_down(&mut self, _: &SelectDown, _: &mut Window, cx: &mut Context<Self>) {
        cx.stop_propagation();
        let first = self.first_selectable();
    ...
    fn _select_submenu(&mut self, window: &mut Window, cx: &mut Context<Self>) -> bool {
        if let Some(active_submenu) = self.active_submenu() {
            ...
            active_submenu.update(cx, |view, cx| {
                let first = view.first_selectable();
                view.set_selected_index(first, cx);
    ...
    fn render_key_binding(
        ...
        .map(|this| {
            if let Some(render) = self.appearance.as_ref().and_then(|appearance| appearance.key.as_ref()) {
                return render(this, window, cx);
            }
            this.p_0()
                .flex_nowrap()
                .border_0()
    ...
    fn render_item(
        ...
        let this = match item {
            PopupMenuItem::Separator => this
                ...
                .disabled(true),
            PopupMenuItem::Label(label) => {
                let this = this.disabled(true).cursor_default();
                if let Some(render) = self.appearance.as_ref().and_then(|appearance| appearance.label.as_ref()) {
                    this.p_0().child(render(label.clone(), window, cx))
                } else {
                    this.child(
                        h_flex()
                            .cursor_default()
        ...
        match (self.appearance.as_ref(), item) {
            (Some(appearance), PopupMenuItem::Separator) => this.refine_style(&appearance.separator),
            (_, PopupMenuItem::Label(_)) => this,
            (Some(appearance), _) => this.refine_style(&appearance.item),
            _ => this,
        }
    }
}
        ...
        v_flex()
            ...
            .child(
                v_flex()
                    ...
                    .p_1()
                    .gap_y_0p5()
                    .when_some(self.appearance.as_ref(), |this, appearance| this.refine_style(&appearance.list))
                    .min_w(rems(8.))
